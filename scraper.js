...
async function runScraper() {
  ...
  // The target table schema is managed outside this scraper.
  // Make sure the existing public.company_full_data_main table matches the columns inserted below.

  let totalScraped = 0;

  try {
    for (let countryIndex = resumeIndex; countryIndex < urls.length; countryIndex += 1) {
      const countryUrl = urls[countryIndex];
      const countryName = countryFromUrl(countryUrl);
      let currentPage = countryIndex === resumeIndex ? resumePage : 1;
      let currentUrl = buildPageUrl(countryUrl, currentPage);
      let hasNext = true;

      console.log(`\nStarting country ${countryIndex + 1}/${urls.length}: ${countryName}`);
      ...
      while (hasNext) {
        ...
        const rows = await scrapePage(page, countryName, currentUrl);
        if (!rows.length) {
          console.log(`No companies found on ${currentUrl}. Ending country ${countryName}.`);
          break;
        }

        for (const row of rows) {
          await retryOperation(() => client.query(
            'INSERT INTO public.company_full_data_main (company_name, location, website, industry, country, source_url, other_data, logo_url) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
            [row.name, row.location, row.website, row.industry, row.country, row.source_url, row.other_data, row.logo_url]
          ));
        }

        totalScraped += rows.length;
        console.log(`  ${countryName} page ${currentPage}: ${rows.length} companies saved. Total scraped: ${totalScraped}`);

        saveProgress({ countryIndex, page: currentPage + 1 });

        const nextPageUrl = await getNextPageUrl(page, currentPage);
        if (!nextPageUrl) {
          console.log(`No more pagination links found for ${countryName}. Moving to next country.`);
          hasNext = false;
        } else {
          currentUrl = normalizeUrl(nextPageUrl);
          currentPage += 1;
        }
      }
    }
    ...
    clearProgress();
    console.log(`\nScraping complete. Total companies scraped: ${totalScraped}`);
  } catch (error) {
